...
void GENERAL_TIM5_IRQHandler(void)
{
  ...
  TIM_ClearITPendingBit(GENERAL_TIM5, TIM_IT_CC1);
	  ...
	  /* 获取输入捕获值 */
  IC1Value = TIM_GetCapture1(GENERAL_TIM5);
  IC2Value = TIM_GetCapture2(GENERAL_TIM5);
	
	  // 注意：捕获寄存器CCR1和CCR2的值在计算占空比和频率的时候必须加1
	if (IC1Value != 0)
  {
		printf("IC2Value=%d",IC2Value);
    /* 占空比计算 */
    DutyCycle = (float)((IC2Value+1) * 100) / (IC1Value+1);

    /* 频率计算 */
    Frequency = (72000000/(GENERAL_TIM5_PSC+1))/(float)(IC1Value+1);
		for(int i=0;i<100000;i++);
		for(int i=0;i<100000;i++);
		
		period = 1.0/(Frequency/1000);
		
		printf("占空比：%0.2f%%   频率：%0.2fHz  周期是：%0.2fMS \n",DutyCycle,Frequency,period);
