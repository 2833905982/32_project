...
/*****************************S1上升套筒********************************/
void S1riseup(void)																										//初值499，末值1499
{
//	TIM_SetCompare1(GENERAL_TIM4,1499);
//	 delay_ms(500);
	for(i = tongxia;i >= tongshang;i--)
	{	
		TIM_SetCompare1(GENERAL_TIM4,i);//对应180度
		delay_us(2000);
	}
}	
/**************************S1下降套筒***********************************/
...
//	for(i = 2499;i >= 499;i--)
//	{	
//		TIM_SetCompare2(GENERAL_TIM4,i);//对应180度
//		delay_us(2000);
//	}
//}
///**************************S2关闭套筒***********************************/
...
//}
/**************************S3上下控制***********************************/
void S3updown(uint16_t j,uint16_t b)									//第一个参数当前位置，第二个参数是要控制到的位置
{
	if(j > b)
 {
	...
	for(i = j;i >= b;i--)
	{	
		TIM_SetCompare3(GENERAL_TIM4,i);//对应180度
		delay_us(2000);																		  //调节延时课控制舵机转动快慢
	}
 }
 	if(j < b)
 {
...
/***************************S4左右控制*************************************/
void S4leftright(uint16_t k,uint16_t a)								//第一个参数当前位置，第二个参数是要控制到的位置
{
	if(k > a)
 {
	...
	for(i = k;i >= a;i--)
	{	
		TIM_SetCompare4(GENERAL_TIM4,i);//对应180度
		delay_us(2000);
	}
 }
 if(k < a)
