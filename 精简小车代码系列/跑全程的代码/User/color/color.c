...
void green(void)
{
	...
	while(1)
	{
		if(TCS230_Distinguish2()==3)					//如果 A 处为红色
		{
			...
			qianjin();
			delay_ms(200);			
			qianjin_lukou1(1);									//前进过一个路口，到达 E 点
			delay_s(1);													//停 1 秒
			
			if(TCS230_Distinguish2()==5)					//如果 E 点为蓝色
			{
				qianjin_lukou1(1);									//前进一个路口，将放在 E 点的物块放到蓝色区
				back_lukou(3);											//后退 3 个路口回到米字
			...
			if(TCS230_Distinguish2()==4)					//如果 E 点为黑色
			{
				back_lukou(2);											//后退 2 个路口回到米字	
				L_zhuanwan1(1);											//向左转，使用的是灰度1，此转弯为 45 度
				qianjin_lukou1(2);									//前进一个路口，将放在 E 点的物块放到黑色区	
				delay_s(1);
				back_lukou(3);											//后退 3 个路口到达米字路口
				R_zhuanwan1(3);											//向右转 3 个路口，准备回家
				delay_s(1);
				qianjin_lukou2(4);									//向前走四个路口，此时车屁股还在出发区外面
				qianjin();													//再向前走一点，回到出发区
				delay_ms(300);
			...
			if(TCS230_Distinguish2()==2)					//如果 E 点为白色
			{
				back_lukou(2);											//后退 2 个路口回到米字	
				L_zhuanwan1(3);											//向左转，使用的是灰度1，此转弯为 135 度
				qianjin_lukou1(2);									//前进两个路口，到达白色区
				delay_s(1);
				back_lukou(3);											//后退 3 个路口到达米字路口
				L_zhuanwan1(3);											//向左转 3 个路口，准备回家
				delay_s(1);
				qianjin_lukou2(4);									//向前走四个路口，此时车屁股还在出发区外面
				qianjin();													//再向前走一点，回到出发区
				delay_ms(300);
