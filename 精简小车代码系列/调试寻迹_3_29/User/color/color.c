...
void green(void)
{
	back_lukou1(3);													//后退过了两个路口，回到米字路口
	delay_s(1);                             //在米字路口停下两秒
	L_zhuanwan(3);													//此转弯使用的是灰度 8 
	delay_s(1);                             //转弯后停两秒
	qianjin_lukou2(2);                      //前进两个路口到达 A 检测区
	...
	while(1)
	{
		...
		{
			qianjin_lukou2(1);									//前进一个路口，到达绿色放置区
			delay_s(1);													//停一秒
//			back_lukou2(4);											//后退，使得小车回到米字的中央，用的第二排灰度
//			turn_right();
//			delay_ms(100);
			back_lukou1(4);
			R_zhuanwan2(3);											//向右转，此时用的是灰度6
			delay_s(1);   											//停 1 秒
			qianjin_lukou2(2);									//前进3个路口，可以到达红色
			delay_s(1);												  // 停1秒
			back_lukou1(4);											//后退3个路口，用的第二排灰度
			R_zhuanwan2(3);											//向右转两个路口，用的灰度9，开始去检测蓝色
			delay_s(1);													//停1秒
//			qianjin();													//前进一会，目的是走过米字，避免此处的干扰
//			qianjin();
//			delay_ms(200);			
			qianjin_lukou2(2);									//前进过两个路口
			delay_s(1);													//停1秒
			back_lukou1(4);											//后退 3 个路口回到米字
//			turn_right();                       //向右转，先盲转一会，使它转过当前可能正压着的线
//			delay_ms(150);
			delay_s(1);
			R_zhuanwan2(3);											//向右转，用的灰度 8 ，使小车开始向出发区前进
			delay_s(1);
			qianjin_lukou2(3);									//向前走四个路口，此时车屁股还在出发区外面
